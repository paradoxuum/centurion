...
import { SUGGESTION_TEXT_SIZE, SUGGESTION_TITLE_TEXT_SIZE } from "../../../constants/text";
...
export function MainSuggestion({ suggestion, currentText, size, sizes, argument }: MainSuggestionProps) {
	const px = usePx();
	...
	useEffect(() => {
		titleHeightMotion.spring(suggestion?.main.description !== undefined ? 1 : 0, springs.responsive);
	}, [suggestion]);
	...
	return (
		...
		>
			...
			<Badge
				...
				size={badgeWidth.map((width) => UDim2.fromOffset(math.round(width), px(24)))}
				position={UDim2.fromScale(1, 0)}
				color={options.palette.highlight}
				text={argument && suggestion !== undefined ? (suggestion.main as ArgumentSuggestion).dataType : ""}
			...
			<Text
				...
				position={lerpBinding(titleHeight, UDim2.fromOffset(0, 0), UDim2.fromOffset(0, px(-4)))}
				text={
					argument
						? suggestion?.main.title
						: highlightMatching(options.palette.highlight, suggestion?.main.title, currentText)
				}
			...
			<Text
				...
				text={argument && suggestion !== undefined ? (suggestion.main as ArgumentSuggestion).error ?? "" : ""}
