...
If a reply is sent in a guard (through `reply` or `error`), the command will not be executed.
...
A guard function is passed the same `CommandInteraction` that will be passed to the command. You can use this to access
data such as the command path or executor and you can send a reply using its `reply` and `error` methods.

A guard function should also return a `boolean` - `true` if the command should be executed, `false` if not.
