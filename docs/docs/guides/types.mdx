...
In this case, you can use the second parameter of `transform` to indicate that the transformation is expensive.

Not specifying a type as "expensive" for types that have slow transformation functions can cause the interface
to lag. This is because a type's transformation function is executed each time a user types a command's argument,
in order to verify the argument's validity.
